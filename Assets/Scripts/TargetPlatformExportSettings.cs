...
public class TargetPlatformExportSettings : EditorWindow
{
    ...
    [MenuItem("Tools/Target Platform Export Settings")]
    static void Init()
    {
        TargetPlatformExportSettings window = (TargetPlatformExportSettings)EditorWindow.GetWindow(typeof(TargetPlatformExportSettings));
        window.titleContent = new GUIContent("Target Platform Export Settings");
        window.Show();
    }

    void OnGUI()
    {
        GUILayout.Label("Target Platform Export Settings", EditorStyles.boldLabel);
        GUILayout.Space(10);
    ...
    private void DisplayPerformanceInfo()
    {
        ...
        GUILayout.Label($"Scene Vertices: {totalVertices}", EditorStyles.helpBox);
        GUILayout.Label($"Render Time: {totalMs:F2} ms", EditorStyles.helpBox);
        GUILayout.Label($"Hypothetical FPS: {hypotheticalFps:F1}", EditorStyles.helpBox);
    }
